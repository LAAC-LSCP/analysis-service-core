"""Word count effort model used as a fast, dependency-free effort model for testing."""
...
class WordCountEffortModel(EffortModel):
    """Effort model for the word count dummy model."""

    def find_igroups(self, dataset_dir: Path) -> List[InputGroup]:
        """Return one input group per .txt file in words/converted/."""
    ...
    def pogroup_from_igroup(
        ...
        """Return the word count file and date file for each input."""
        converted_dir = dataset_dir / "words" / "converted"
        pogroup = []
        for f in igroup:
    ...
    def ogroup_from_pogroup(
        ...
        """Return only the word count files, excluding the temporary date file."""
